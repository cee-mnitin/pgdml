...
class CabDriver():
    ...
    def update_day_and_time(self,time,day,time_taken):
    ...
    def reward_func(self, state, action, Time_matrix):
        ...
        if pickup == 0 and drop == 0:
            ...
            reward = (R*Time_matrix[pickup, drop, int(time_next),int(day_next)] - C*(Time_matrix[pickup, drop, int(time_next), int(day_next)] + Time_matrix[start_loc, pickup, int(time), int(day)]))
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        if cab_pos!=pickup_pos: #Calculate the new time of day and week when cab reaches pickup position
            ...
            new_time_of_day,new_day_of_week = self.update_day_and_time(time_of_day,day_of_week,time_taken)
            total_time += time_taken
        
        if (pickup_pos == 0) and (drop_pos==0):
            total_time += 1
            new_time_of_day,new_day_of_week = self.update_day_and_time(time_of_day,day_of_week,1)
            ...
            total_time += time_taken
            final_time_of_day,final_day_of_week = self.update_day_and_time(new_time_of_day,new_day_of_week,time_taken)
